...
class Logger(object):
    def __init__(self, save_dir):
        ...
        self.loss_log = open(save_dir + "loss_log.txt", "w")  # Open a text file for loss values
        self.dsc_log = open(save_dir + "dsc_log.txt", "w")  # Open a text file for DSC values
    ...
    def log_loss(self, loss): #
        self.loss_log.write(str(loss) + "\n")

    def log_dsc(self, dsc):
        self.dsc_log.write(str(dsc) + "\n")
...
def main():
    ...
    head_dim = 6
    num_heads = [8,4,2,1,1]
    save_dir = 'MDH_ncc_{}_reg_{}_lr_{}/'.format(weights[0], weights[1], lr)
    ...
    Initialize model
    '''
    model = MDH(img_size, head_dim=head_dim, num_heads=num_heads, scale=1)
    model.cuda()
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        # Log the DSC value in the text file
        sys.stdout.log_dsc(eval_dsc.avg)
